...
*///////////////////////////////////////////////////////
float control_exoesqueleto(float *leapmotion, float Ts, float *data_config);
...
int main(int argc, char const* argv[])
{
    ...
    while(1){
        ...
        while(flag_continue){
            ...
            torque_control= control_exoesqueleto(datos_entrada, TIEMPO_MUESTREO,  num_in_python);
            printf("Salida estimada: %f\n",torque_control);
            ...
            if(num_in_python[5]==1){
                ...
                vec_out_socket[0]= id;
                vec_out_socket[1]=ambiente_rv;
                vec_out_socket[2]=disposicion;
                vec_out_socket[3]=intensidad_torque;
                vec_out_socket[4]=torque_control;
                vec_out_socket[5]=torque_final;
                vec_out_socket[6]=9.8;


                tam_buffer = vec2str(buff_send, vec_out_socket, 7);
                ...
                vec_out_socket[3]=intensidad_torque;
                vec_out_socket[4]=torque_control;
                vec_out_socket[5]=torque_control*intensidad_torque;
                vec_out_socket[6]=9.8;
                id+=1;
                tam_buffer = vec2str(buff_send, vec_out_socket, 7);
                send(sock, buff_send, (tam_buffer), 0);
